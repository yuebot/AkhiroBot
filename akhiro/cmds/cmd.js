  ...
  onRun: async ({ api, event, args, fonts }) => {
    ...
    if (args.length === 0) {
      ...
      switch (subcommand) {
        ...
        case "install":
          if (args.length >= 3) {
            ...
            try {
              ...
              api.sendMessage(
                `✅ | Command '${filename}' installed successfully!`,
                event.threadID,
              );
            } catch (error) {
              api.sendMessage(
                `❌ | Error installing command: ${error.message}`,
                event.threadID,
              );
            }
          } else {
            api.sendMessage(
              `❌ | Specify a code link and filename to install a command.`,
              event.threadID,
            );
          }
          break;
        ...
        default:
          api.sendMessage(
            `❌ | Unknown subcommand. Use 'help' for assistance.`,
            event.threadID,
          );
      }
    }
