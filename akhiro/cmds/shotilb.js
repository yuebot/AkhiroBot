    ...
    try {
      ...
      if (method === "leaderboard" || method === "stats") {
        const response = await axios.post(
          "https://shoti-srv1.onrender.com/api/info",
          { f: method },
        );
        ...
        api.sendMessage(
          `${fonts.applyFonts("❌ Invalid method. Use 'leaderboard' or 'stats'.", "bold")}`,
          event.threadID,
        );
      }
    } catch (error) {
      console.error(error);
      api.sendMessage(
        `${fonts.applyFonts("❌ An error occurred while fetching data.", "bold")}`,
        event.threadID,
      );
    }
  },
