...
module.exports.onRun = async function ({
  ...
  if (!args[0]) {
    api.sendMessage(`Usage: ${global.AkhiroBot.botPrefix}tik video <link>\n${global.config.PREFIX}tik audio <link>`, event.threadID, event.messageID);
    ...
    try {
      ...
      const response = await axios.get(`https://eurix-hiyoshi-apis.replit.app/tiktokdl?url=${url}`);
    ...
    api.sendMessage(`Invalid command. Usage: ${global.AkhiroBot.botPrefix}tik video <link>\n${global.AkhiroBot.botPrefix}tik audio <link>`, event.threadID, event.messageID);
  }
};